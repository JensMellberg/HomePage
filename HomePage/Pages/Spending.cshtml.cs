...
{
    public class SpendingModel(AppDbContext dbContext, SpendingGroupRepository spendingGroupRepository, SignInRepository signInRepository) : BasePage(signInRepository)
    {
        ...
        {
            Person = person ?? "Both";
            var allSpendings = dbContext.SpendingItem.Where(x => x.Person == Person).ToList();
            
            var allDates = allSpendings.Select(x => new DateTime(x.TransactionDate.Year, x.TransactionDate.Month, 1)).Distinct().OrderBy(x => x).ToList();
            ...
            var groups = spendingGroupRepository.GetAllSpendingGroups(Person, convertedSelectedDate, convertedSelectedDate.AddMonths(1).AddDays(-1));
            GroupOptionsJson = JsonConvert.SerializeObject(groups.Where(x => x.Id != Guid.Empty).Select(x => new { id = x.Id, label = x.Name }));
            var spendingsForMonth = allSpendings.Where(x => x.TransactionDate.Month == convertedSelectedDate.Month && x.TransactionDate.Year == convertedSelectedDate.Year).ToList();
        ...
        public static IDictionary<string, MonthSpendingGroup> SortIntoGroups(IEnumerable<SpendingItem> items, IEnumerable<ISpendingGroup> groups)
        {
            ...
            foreach (var spending in items)
            {
                ...
                {
                    monthGroups.Values.Where(x => x.Id == Guid.Parse(spending.SetGroupId)).First().Entries.Add(spending);
                    continue;
                }
